...
None of it takes prompt engineering — you describe the thing and answer plain questions; the vocabularies carry the hundred small decisions. And the things it makes aren't a menu of separate features — they're a **ladder**: a chatbot becomes a connected service when you wire in your MCPs, apps and services become experiences, worlds and music become a game's assets, and a game composes the whole stack. Each rung is a tiny deterministic **recipe** — small enough to read, seeded to regenerate identically, yours to edit a line at a time — so a world you walked becomes a game's level and a groove becomes its score. Your agent is the one thing that climbs the ladder, which is why it stays one substrate, not a suite you'd integrate yourself.
...
<sub>One of the worlds your agent composes from primitives — rendered without a game engine, served as a plain HTML scene. The workshop makes bots, apps, services, publications, and worlds; this is a world. See [Worlds, media &amp; visuals](#worlds-media--visuals).</sub>
...
- **Sketches & worlds** — the visual bay. Two-dimensional diagrams (flowcharts, stacked bars, donuts, KPI tiles, decision diamonds via `create_sketch`) *and* generative 3D: cities, posed figures, painted landscapes, carved wordmarks, transit hubs, everyday objects, and drivable worlds. One geometry spec renders as an SVG, a dependency-free CSS-3D scene, a traversable WebGL world, or a `.glb`. Minted by your agent, served under `/sketches/<ref>`. See [Worlds, media & visuals](#worlds-media--visuals).
...
## Bots — the deployable pillar

Bots are the one bay that compiles to a standalone artifact built to face real end users: its own process, its own database, deployable to the cloud. It's stayed one of the deepest surfaces as the rest of the workshop grew up around it:
...
## Worlds, media & visuals

Most of the workshop now lives here — it's the largest, deepest surface. The same `create_*` family that draws a flowchart also composes generative 3D, synthesized audio, and full films — geometry, sound, and motion built from primitives on your machine, no image model, no cloud render, **no provider key**:
...
Because it's deterministic geometry, the first render costs nothing but the render — this is the keyless first look in the [Quickstart](#quickstart). And live *rule-driven* worlds are no longer a further layer — the same geometry becomes a game's level. That's the next section.

---

## Games — composition, made playable

A game is the top rung: not a new kind of media beside the others, but a composition **of** them. `create_game` binds worlds, music, figures, and sprites by reference into a standalone playable artifact — a shell that owns a typed **store** and a set of **levels** (worlds minted with a `game:` contract). Improve a bound world or score and the game inherits it.
