...
The dashboard at `localhost:3001` is a shelf of ten bays. Your agent fills them:
...
- **Operations** — scope a domain of work and read its state: tag related artifacts (bots, apps, services, cooks) into one operation. A consulted map, not a driver.
...
Because it's deterministic geometry, the first render costs nothing but the render — this is the keyless first look in the [Quickstart](#quickstart). (Live *rule-driven* worlds — scoreboards, spawns, hit detection, a game loop — are a further layer, not covered here yet.)
