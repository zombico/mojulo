...
/**
 * /games/[ref]'s client body — the game-developer studio proper: one project's
 * assets behind the workshop tile primitive — icon tabs (the same ws-tile
 * boxes as the /maker hub, studio hue) select a category, the category's
 * assets list below in a centered vertical column, and a MODAL viewer opens
 * one asset in context (world frame, beats player) or shows why it can't.
 * Folders group by what a thing IS: the game manifest's own levels and music
 * merge into Levels/Audio beside bound candidates — provenance is a badge,
 * not a location. The game itself never previews in-page: its tab is a
...
export default function GameDeveloperBody({ projectRef: ref, authEnabled = false }) {
  ...
  const shellProps = {
    posture: 'pinned',
    width: 1600,
    crumb: t('galleryTitle'),
    authEnabled,
  };

  if (notFound) {
    return (
      <WorkshopShell {...shellProps}>
