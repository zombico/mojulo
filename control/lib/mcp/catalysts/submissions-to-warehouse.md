---
{
  ...
  "parameters": [
    ...
    },
    {
      ...
      "prompt": "On the first run, should the artifact backfill all historical submissions, or start from now-forward only? Backfill is fine for low-volume bots; bounded windows are safer for high-volume.",
...
2. Ask the user the four `parameters` questions in one batched round. Propose a default `columnMapping` derived from the form schema (best-guess types per field name) and let the user adjust — don't make them type the whole mapping from scratch.
...
4. Hand the resolved workflow (inputs `deploymentId` + `since`, schema, incremental cursor logic, bulk/streaming choice) to the host adapter to materialize the runnable artifact.
...
The schema should be **additive-friendly** — when the bot's form grows new fields, the artifact should detect the unmapped field, route it to `raw_extras`, and emit a clear log line. The user can later promote it to a typed column with an ALTER + a one-time backfill from `raw_extras`. Don't try to auto-ALTER the schema from the artifact; warehouse DDL is operator territory.
...
4. Surface the new high-water timestamp per the host adapter's reporting rules so the user (or a scheduler) can pass it back.

The artifact should NOT rely on `submission_id` for incremental cursoring — IDs aren't guaranteed monotonic over time in the bot's SQLite. Always use timestamp.
...
- **PII in the warehouse.** Warehouses are typically more broadly accessible than the bot's SQLite — analysts, BI tools, downstream pipelines, sometimes vendors. If the form captures sensitive fields (DOB, SSN, financial, medical), the artifact should default to **excluding** those columns from the mapping and pointing the user at column-level encryption or a separate restricted-access table. The user can override, but the question must be asked explicitly during synthesis.
- **Schema drift.** When the bot's form gains/renames/drops fields, the warehouse columns silently misalign. The artifact must detect unmapped fields each run and emit a log line; recommend the user re-run the catalyst flow when they change the form.
