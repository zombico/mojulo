...
export async function createFigureHandler(input) {
  ...
  if (garment !== undefined && garment !== null) {
    // a single key OR an array (layering: e.g. ['trousers','jacket'] worn together)
    const keys = Array.isArray(garment) ? garment : [garment];
    const bad = keys.find((k) => !GARMENT_KEYS.includes(k));
    if (bad !== undefined) throw new Error(`\`garment\` must be one of ${GARMENT_KEYS.join(', ')} (a key, an array of keys to LAYER, or null) — got '${bad}'`);
  }
  ...
  registerTool({
    ...
    inputSchema: {
      ...
      properties: {
        ...
        garment: { type: ['string', 'array', 'null'], items: { type: 'string', enum: GARMENT_KEYS }, description: `Clothing over the body (tracks pose + build, body-relative cuts/panels/seams carve to fit any form): one of ${GARMENT_KEYS.join(' | ')}, an ARRAY of keys to LAYER them (e.g. ['trousers','jacket']), or null = bare.` },
