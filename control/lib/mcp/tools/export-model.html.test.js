...
describe('export_model format:html', () => {
  it('writes a self-contained, deterministic world.html beside recipe.json and README.md', async () => {
    ...
    // RE-PINNED 509 → 647 → 685 with the fractal-city `frontage` channel: a NEW mint writes
    // `elements.frontage: true` into its manifest (the one new-mint default that differs from a
    // stored row); the parking-entrance dressing on the large masses is 138 boxes here and the
    // dead-end drop-off aprons (islands, bollards, planters) another 38. The maintainer's stored
    // seed-91 row (no flag) still plans the 509 asserted below — nothing about the city's masses,
    // roads or blocks moved.
    expect(minted.recipe.elements.frontage).toBe(true);
    expect(minted.stats.boxes).toBe(685);
