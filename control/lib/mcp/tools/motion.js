...
export async function renderShot({ subject, shot }) {
  ...
  if (isWorld) {
    if (isDeckMotion(motion)) {
      throw new Error("'deck' needs a deck subject — a world animates with a camera motion (turntable/orbit/push_in/dolly_zoom/flythrough).");
    }
    if (!WORLD_MOTION_NAMES.includes(motion)) {
      throw new Error(`unknown world camera motion '${motion}'. One of: ${WORLD_MOTION_NAMES.join(', ')}.`);
    }
  ...
  registerTool({
    ...
    inputSchema: {
      ...
      properties: {
        ...
        export: { type: 'string', enum: ['gif', 'svg', 'mp4', 'both'], description: "Artifact form (default 'both'). SVG families: 'svg' is the durable flipbook, 'gif' the cache. WORLD family (raster-native, no svg): 'gif' is the looping preview, 'mp4' the downloadable H.264, 'both' writes both." },
