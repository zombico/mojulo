...
const LEXICON = [
  ...
  },
  {
    ...
    support: PARTIAL,
    routes: [
      // parts-booleans.plan.md B2: the `cuts` line first — a hole through parts already composed as monomers
      { tool: 'mint_solid', when: "a hole through a part you composed from monomers: give the body and the cutters an `id` and name them in `cuts` — the workbench rewrites them into ONE field solid (body `add`, cutters `subtract` / `intersect`), keeps the body's material, and the recipe stays lathes + sweeps + the cut, so a moved bore re-cuts on update_sketch. Reaches lathes, solid extrudes, sweeps, fields; `cells` is the edge dial.", args: { kind: 'workbench', spec: { units: 'cm', lathes: [{ id: 'disc', axisFrom: { x: 0, y: 0, z: 0 }, axisTo: { x: 0, y: 0, z: 1.2 }, profile: [{ t: 0, radius: 6 }, { t: 1, radius: 6 }] }], sweeps: [{ id: 'bore', path: [[0, 0, -1], [0, 0, 3]], radius: 1.2 }], cuts: [{ from: 'disc', subtract: ['bore'], cells: 96 }] } } },
      { tool: 'mint_solid', when: "the workbench `fields` monomer, written by hand: a term list read top-down — `add` a shape, `subtract` a shape (a bore is a `sweep` term cut from a `lathe` term), `blend` for a filleted rim; the form `cuts` lowers to, for when the body is itself a field composition.", args: { kind: 'workbench', spec: { units: 'cm', fields: [{ cells: 96, terms: [{ id: 'disc', op: 'add', shape: { kind: 'lathe', axisFrom: [0, 0, 0], axisTo: [0, 0, 1.2], profile: [{ t: 0, radius: 6 }, { t: 1, radius: 6 }] } }, { id: 'bore', op: 'subtract', shape: { kind: 'sweep', path: [[0, 0, -1], [0, 0, 3]], radius: 1.2 } }] }] } } },
    ],
    then: [
      ...
      { tool: 'update_sketch', when: 'the SHARP EDGE inside the recipe: set `exact: true` on the `fields` entry or the `cuts` entry (Manifold composes the same terms; a bore gets a true circular lip in /world, the .glb and the print). Refused by name when a term has no exact twin (blend, stroke, displace, shell, round, expr, warps).', args: { ref: '<sk_ref>', patch: [{ op: 'set', path: '/cuts/0/exact', value: true }] } },
    ...
    ceiling: "Field-space booleans on the surface net: exact sign, closed by construction, genus-correct — but every edge rounds to about one grid cell (`cells` 16–128; cost is cubic). `exact: true` on the field or the cut composes the same terms with Manifold IN the recipe: a sharp lip, exact size, everywhere the row renders — for the nine shapes and unblended booleans, transform and repeat. A blend, a stroke, noise, an expr or a warp stays sampled (and arrives frozen in a `scad` export — the ledger names each one); two more exits are `union: true` at export and `format: 'scad'` for OpenSCAD.",
