...
- Plan non-trivial work → \`enter_plan_mode\`. Gather / research broadly → \`enter_research_mode\` (for new gatherings prefer the typed-intake path: \`mint_stash\` → \`gather\` → optional \`cook\`; both low-prominence; reach for them when asked). A cook outcome can later seed plan mode via \`forge_plan({ source: { kind: 'cook', cook_ref } })\` — cook stops at cook; the bridge is plan-side.
