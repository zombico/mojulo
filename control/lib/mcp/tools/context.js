...
- \`create_polygonized_sketch\` — generate a sketch from a natural-language visual prompt. \`mode: 'one-trip'\` (default; flat scenes — portraits, charts, single figures) or \`'plan-then-skin'\` (two-turn; perspective / multi-figure / architectural — plans + scaffolds before marks). Calls a provider LLM (defaults to the saved key, or ollama); no key? — \`get_polygonizer_packet\` hands YOU the same discipline to author yourself.
- \`get_polygonizer_packet\` — the KEY-FREE twin of \`create_polygonized_sketch\`: YOU are the generative model. Returns \`{ instructions, userPrompt, schema, submit }\` — adopt the instructions, author ONE JSON manifest, hand it to \`submit_polygonizer_manifest\`. Same \`mode\` + \`preload\` knobs.
- \`submit_polygonizer_manifest\` — submit half of the key-free loop: validates, applies deterministic repairs, lowers, mints → \`{ ok, sketch: { ref, url } }\`; on failure returns \`repairPrompt\` — fix your manifest and resubmit. Plan-then-skin \`phase: 'planning'\` returns the solved \`scaffold\` + \`skinPacket\` for the marks turn.`,
  },
  'illustration': {
