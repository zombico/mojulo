...
- **Mint a visual** (flow chart, data chart, scene illustration, painterly landscape) → \`create_sketch\` for diagrams (chart vocab via \`semantic_search({kinds:['sketch_vocab']})\` + \`get_sketch_vocab\`); \`update_sketch\` to iterate in place. Scene/figure illustration → \`sketch_what_possible\` knob-resolution loop → \`create_sketch({recipe:{kind,...}})\`, or single-shot \`create_polygonized_sketch\` for natural-language prompts. Cardinal-grammar structural illustration → \`create_manji_tree\` (2D or 3D). Painted landscape from glyph choices → \`create_painted_landscape\`. Compare two diagrams → \`diff_sketches\`. Sketches persist at \`/sketches/<ref>\` and embed into picture-book / comic / field-guide / visual-guide cooks via \`sketch\`-typed stash items.
