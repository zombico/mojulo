 ...
 *   operations-mode.js, and this file. If you add or remove a tool, update the relevant rows here
...
- \`get_tool_telemetry\` — the substrate's own tool-call telemetry. No args → per-tool aggregate table (calls, error rate, p50/p95, last-called) over the last N days + recent errors/timeouts; \`{ tool }\` → that tool's recent calls. The in-session answer to "which tool broke?" / "what's slow?". Records shapes only, never values. Mirrors the \`/observability\` page.
...
- \`get_game_vocab\` — read a game-vocab card in full: state shape + the typed events it accepts + level consume/produce wiring for one store slice kind (slice-character / slice-inventory / slice-party / slice-progression / slice-flags), or the typed-events reference. Omit \`id\` for the index rows. Pair with \`semantic_search({ kinds: ['game_vocab'] })\`.
...
- **Mint a visual** (flow chart, data chart, scene illustration, painterly landscape, posed figure, carved 3D solid, generated worlds, animated study objects) → \`create_sketch\` for diagrams (chart vocab via \`semantic_search({kinds:['sketch_vocab']})\` + \`get_sketch_vocab\`); \`update_sketch\` to iterate in place. Scene/figure illustration → \`sketch_what_possible\` knob-resolution loop → \`create_sketch({recipe:{kind,...}})\`, or single-shot \`create_polygonized_sketch\` for natural-language prompts. Cardinal-grammar structural illustration → \`create_manji_tree\` (2D or 3D). Painted landscape from glyph choices → \`compose_world\` (base \`painted-landscape\`; find a sky/palette/geometry glyph by intent via \`semantic_search({kinds:['painted_landscape']})\`, pass glyph ids in \`overrides\`). **Pose a human body** ("pose a figure / a person reaching / a walking figure / a male\\|female body in a stance / a figure wearing X") → \`create_figure\` (pose / build / garment / view dials; \`motion:'walk'\` adds a gait GIF). **A 3D metal/carved wordmark, logo, or badge** ("chrome text / extruded logo / beveled icon / metalify this shape") → \`create_carved_solid\`. **A generated 3D city / skyline / airport / train station / bus terminal / subway / transit hub** → \`compose_world\` (base \`city\` or \`transport-hub\`; tiny seed recipe → a live CSS preserve-3d scene at \`/api/sketches/<ref>/scene\`; theme packs via \`list_world_themes\`). **A single everyday OBJECT / mechanical part rendered from primitives at literal scale** ("render/turntable a candlestick / bottle / dumbbell / vase / lamp; block out an object in solids") → \`create_workbench\` (a polygomer of \`lathes\` → a measured object served traversable at \`/world\` + preset shots at \`/scene\`). **A LIVE / walkable / playable world** ("walk a character through X / fly a drone / a platformer / make a game where… / whack-a-mole / a score and a timer / show me my services as a world") → \`compose_world\` (base \`controllable\` for drive/fly/platform, \`action\` for games with rules — an \`idioms\` recipe in \`overrides\` — \`operator\` for mojulo's own connected-services state, \`planetary\` for a space-accurate body; served traversable at \`/world\`; read the base's manual via \`get_view_vocab({id:'<base>'})\`). **An animated science / math / bio STUDY OBJECT** ("show/teach me nuclear fission / the double-slit experiment / what a derivative is / DNA / a black hole") → \`create_view\` (43 kinds; find one by intent via \`semantic_search({kinds:['view_vocab']})\`, read its parameter manual via \`get_view_vocab\`, then pass \`kind\` + \`params\`; served live at \`/world\`). Compare two diagrams → \`diff_sketches\`. Sketches persist at \`/sketches/<ref>\` and embed into picture-book / comic / field-guide / visual-guide cooks via \`sketch\`-typed stash items.
...
- **Replay a RUN through a world / prove it plays** ("walk through the dungeon and film it / record a tour of the city / show the character jumping the gap / play the run back / prove the level is beatable / test-drive the world") → \`forge_motion\` TRAVERSAL: \`subject.world_ref\` + \`shot.motion:'traversal'\` + \`shot.ticks\` (one input snapshot per tick — {forward,strafe,jump,lookDX,…} at \`shot.fps\`). Drives the world's LIVE channels (controllable entities / physics / events) deterministically: same ticks → identical run. Yields the MP4/GIF of the run AND a per-tick probe stream (entity positions, HUD vars → \`probes.json\`, final probe in the result) to assert outcomes against. Camera: the world's camera entity (follow/FPV), or inject one via \`shot.params.camera\`. (Contrast: a camera flying over a self-animating world → the WORLD camera motions.)
- **Walk to a PLACE in a world / verify it's walkable** ("walk to the exit / send the character to X then Y / can you reach the platform / verify the world is traversable / audit walkability") → \`forge_motion\` TRAVERSAL with \`shot.waypoints\` (a [x,y] route) instead of hand-authored ticks: each leg is COMPILED into the tick script against the live walk/platform rule (closed-loop steering; no pathfinding — a blocked/void leg reports \`{stuck, atTick}\` in the recipe's \`legs\` and the final probe shows where the route died). The compiled ticks are stored, so the recipe replays deterministically like any traversal. Compile entrance→exit + check \`legs\`/final probe = the walkability audit.
...
- **Make a GAME / compose levels into a playable whole** ("make a game", "a tactics game with a persistent army", "a dungeon crawler where loot/level carries between levels", "a campaign with unlockable levels", "a pre-level loadout screen", "a customizable squad that persists") → \`create_game\`: a SHELL owning a typed STORE (from five slice kinds: character | inventory | party | progression | flags) + a list of promoted LEVELS. A level is a world you minted with a \`game:\` contract channel (\`compose_world\` … \`{ game: { levelRef, consumes, produces } }\`) declaring which store slices parameterize it and which typed events its one outcome may write back. The shell renders each level's setup screen from \`consumes\` (a \`pick\` = a loadout/roster picker), hosts the level, and applies its outcome to the store; state carries between levels; play data never enters mojulo. Design the store first via \`semantic_search({kinds:['game_vocab']})\` + \`get_game_vocab\`. A level is only promoted once it passes a contract dry-run AND is shown completable (an \`audits\` map of \`forge_motion\` traversals that reached each level's win condition; \`allow_unaudited:true\` to skip). Played at \`/sketches/<ref>\`. The fourth creatable paradigm, sibling to bots / connected services / apps.
...
- **\`/maker\`** — Mojulo Maker, the illustration concern and a sibling to Sketches (its own tuned, opinionated renderer). Four rails: **Illustrations** (\`/maker/illustrations\` — landscapes, figures, and complicated perspective/css3d/painterly renders), **Worlds** (\`/maker/worlds\` — traversable three.js cityscapes/hubs), **Motion** (\`/maker/motion\` — movies & gifs; \`/motion\` redirects here), and **Beats** (\`/maker/beats\` — the audio shelf: synthesized soundtracks / compositions / sound-effect cues from \`create_beats\`, previewed in a live player; browse-and-play only — authoring stays with the host agent). An illustration is the SAME sketch primitive as a diagram, bucketed by \`manifest.kind\` — you can still stash, reference, and diff it. The bucket only decides which sibling concern owns it.
...
export function registerContextTools() {
  ...
  registerTool({
    ...
    description:
      "Read the MCP tool-layer telemetry the substrate records for its own tool calls (one row per handler invocation across both call paths — rpc + plan-executor). Two modes: no args → a per-tool aggregate table (calls, error rate, p50/p95 latency, last-called) over the last `sinceDays` (default 7) plus the most recent errors and timeouts; `{ tool }` → that tool's recent calls newest-first with status + duration. This is the in-session answer to \"which tool broke?\" / \"what's slow?\" without leaving the chat. Records SHAPES only — never input values or conversation content. Read-only, idempotent. Mirrors the `/observability` dashboard page.",
    inputSchema: {
      type: 'object',
      properties: {
        tool: {
        ...
        sinceDays: {
          type: 'number',
          description: 'Aggregate window in days (no-args mode only). Default 7.',
        },
        limit: {
          type: 'number',
