...
- \`recommend_mcp_orbit_compositions\` / \`get_meta_catalyst\` / \`list_mcp_orbit_components\` / \`get_mcp_orbit_component\` — **the mcp-orbit composer** sits on top of \`meta_context_declare_inventory\`. Once inventory is declared, mcp-orbit decomposes the no-bot workflow space into five typed component kinds (\`mcp\` × \`trigger\` × \`pattern\` × \`idempotency\` × \`render\`) that the agent combines under the meta-catalyst's discipline. Each \`mcp\` component declares an \`affordances\` map; \`source\` and \`destination\` are composition ROLES tagged per entry in \`component_refs\` (so the same MCP can play either role depending on the composition). The flow is fixed: \`recommend_mcp_orbit_compositions\` (logs the candidates as audit-able \`proposed\` rows) → \`get_meta_catalyst\` (composition rulebook, read once per session) → \`get_mcp_orbit_component\` per chosen ref → assemble + dry-run + \`meta_context_commit\`. Use when the user wants a workflow that reads from one MCP and writes to another ("weekly Linear digest into Drive", "route Gmail support threads into Linear", "summarize closed issues into a channel"). Distinct from the bot-shaped \`recommend_catalysts\` flow — that recommends against a deployment; this recommends against the operator's declared MCP inventory with no bot in the picture.
- \`bind_primitives\` — **experimental primitive-binding path (parallel to the mcp-orbit composer above).** Given a vendor-agnostic primitive (\`document-store\`, \`issue-tracker\`, ...), a composition role (\`source\` | \`destination\`), and a server from declared inventory, runs a deterministic generator that fills a role-specific template with the **actual bound tool names + schemas from the operator's installed MCP**. Returns a session-scoped provider artifact (\`prov_<id>\`) + structured binding manifest (which affordances mapped to which tools, with confidence labels). Use when inventory was declared in "richer-snapshot mode" (per-tool \`inputSchema\` + \`introspectionConfidence\`); thin inventory declarations downgrade to \`names_only\` confidence with no schemas in the generated artifact. The bound provider artifacts then graduate via \`meta_context_commit({type:'primitive_artifact_materialization', adapter_id, artifact, composition_intent, provider_artifact_refs:[...]})\` — a sibling commit path that records the audit chain (artifact → bound MCP tools, with per-binding payloads) without requiring a bot or catalyst. The vendor-shaped \`recommend_mcp_orbit_compositions\` flow is the supported default; \`bind_primitives\` is the runtime-introspected alternative we're validating in parallel — prefer it when you have rich tool-schema knowledge and want the generated artifact to reflect the operator's actual installed MCP rather than a curated guess. See lite-template/integration/MCP_PRIMITIVE_BINDING_PLAN.md.
...
// CAUTERIZE banner — when MOJULO_MCP_ORBIT_VENDOR_DISABLED=1, prepend a directive
// telling the agent that the vendor-shaped mcp-orbit composer is unavailable and
// to drive all MCP-to-MCP workflow reasoning through `bind_primitives` instead.
// Matches the registration gate in server.js. See
// lite-template/integration/MCP_PRIMITIVE_BINDING_PLAN.md.
const VENDOR_DISABLED_BANNER = `> **Session note — vendor-shaped mcp-orbit composer is disabled.**
>
> The tools \`recommend_mcp_orbit_compositions\`, \`list_mcp_orbit_components\`, \`get_mcp_orbit_component\`, and \`get_meta_catalyst\` are NOT registered in this session. References to them in the body below should be ignored.
>
> For all MCP-to-MCP workflow reasoning, use the **primitive-binding flow**: decompose the operator's intent into primitive slots (\`document-store\`, \`issue-tracker\`), call \`meta_context_declare_inventory\` in richer-snapshot mode (per-tool \`inputSchema\` + \`introspectionConfidence\`), call \`bind_primitives\` once per slot, then commit via \`meta_context_commit({type:'primitive_artifact_materialization', ...})\`. The three host adapters (\`get_adapter\` → \`claude-code\` / \`codex\` / \`generic\`) document the substrate-specific materialization. See lite-template/integration/MCP_PRIMITIVE_BINDING_PLAN.md.
...
export async function forwardContextHandler(_input, _ctx) {
  ...
  const body =
    process.env.MOJULO_MCP_ORBIT_VENDOR_DISABLED === '1'
      ? VENDOR_DISABLED_BANNER + FORWARD_CONTEXT_BODY
