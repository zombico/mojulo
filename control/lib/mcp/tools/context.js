...
**Bots are an optional pack.** The chatbot factory is install-gated (\`chatbot\`); if it is absent on this host its tools neither list nor run, and the advisory points at the install. The embedding model behind vector \`semantic_search\` is a second opt-in group (\`recall\`, \`mojulo install recall\`): without it search still answers, ranking lexically, and a result's \`mode\` says which ran. Everything else — the kernel, the creative studio, and the automation backend — is always present, on the operator's machine or in the agent's own temporary box (Claude Code on the web with \`npx -y mojulo init --yes --no-ui\`, or a shell-only sandbox such as Grok chat's with \`npx mojulo call\`; results handed back as files — every written export carries a \`handoff\` naming this host's door, and \`format: 'bundle'\` is the one-zip handoff). Do not describe mojulo as a bot factory.
...
- A house / apartment / cottage / one furnished room is NOT a base — it is \`create_sketch\` with \`manifest: { kind: 'floorplan', … }\` (the diagram form's tool; walkable, furnished, \`storeys: N\`; card \`get_sketch_vocab({ id: 'floor-plan' })\`). \`export_model\` above exports it; \`lit: true\` gives real PBR materials an engine lights.`,
  },
  'view': {
...
mint dispatch — match the ask to a FORM:
  ...
  PICTURE  diagram / data chart ("draw me X", "chart these numbers") → \`create_sketch\` · scene / figure illustration ("illustrate X", a portrait, a landscape) → \`sketch_what_possible\` (its \`architecturalConstruction\` family draws a house's FACADE — a picture, not a place; a house to walk is HOUSE) · a posed person / figure / non-humanoid creature (even asked for as "a picture of…") → \`mint_solid\` · build from a PHOTO you can see → \`reference_protocol\` · direct an AI-generated image / comic page → \`create_sketch\` kind 'image-outcome'/'sequential-art'
  ...
  HOUSE    a house / apartment / cottage / townhouse / office floor plan / ONE furnished room — walkable, furnished by room glyph, stacked with \`storeys: N\` → \`create_sketch\` with \`manifest: { kind: 'floorplan', … }\` (card \`get_sketch_vocab({ id: 'floor-plan' })\`; NOT compose_world, NOT edifice)
  BUILDING institutional / bespoke one-off (a terminal, a hall, a block of masses + concourses) → \`mint_solid\` kind edifice · a dwelling is HOUSE
