...
### Deliberation (Ring 6 — record WHY structural decisions were made)

Mojulo separates *what fired* (a conversation, an automation run — outcome-rate, never written here) from *why it was bound this way* (a catalyst materialized through a host adapter into an artifact running against a bot — deliberation-rate, written here). \`meta_context\` is the writeable, durable layer for the second kind. Two tools, both rare-call by design — expect 0–3 calls per session, each followed by something load-bearing.

- \`meta_context_brief\` — read the contextmap subgraph + principles for a scope (\`{ kind: 'fleet' }\` for the whole graph, or \`{ kind: 'bot' | 'catalyst' | 'adapter' | 'artifact', ref: '<id>' }\` for a 1-hop neighborhood). Call when wondering *"has the fleet already committed to something related to what I'm about to do?"* or when the user asks "why does bot-3 route field X to tool Y?" / "why is this a Codex automation and not a skill?" — the \`materialized_by\` and \`binds\` edges carry principles that record the reasoning. An empty fleet brief returns \`meta: { empty: true, suggest_kyc: true }\` — surface the operator KYC at that point. Do NOT call for routine orientation (that's \`forward_context\`), operational metrics (\`fleet_*\`), or content questions (\`operate.*\`).
