...
**Two pipelines, one honesty rule.** DIGITAL: worlds / games / scenes feed Godot, Unity, Unreal, and Blender. Godot is first-class (a real project plus a headless machine gate); the Unity and Unreal legs ship as a data pack plus an importer (Unreal adds a C++ kernel plugin), each with an advisory gate that runs only when \`MOJULO_UNITY\` / \`MOJULO_UNREAL\` names a binary; Blender takes an art-pass pack and an optional GI bake into vertex colours. Ladder honestly ("Godot first, Unity and Unreal as gated legs"), never "identical across all four." PHYSICAL: the literal profile (workbench / assembler / carved wordmark / turntable / vehicle) exports as print-purposed STL or 3MF at true scale — mm, z-up, slicer-ready; figures, worlds and views print as maquettes fit to a target size (default 120 mm), so say "true scale" only for the literal profile. Claim **"print-ready STL at true scale"**, NOT "guaranteed watertight manifold" (it emits honest triangle soup and relies on slicer repair, which is standard practice). A third physical exit is \`format: 'scad'\` — the recipe TRANSPILED into an OpenSCAD program rather than serialized as a mesh, so OpenSCAD's exact booleans give the sharp edges a sampled field cannot (every field edge rounds to about one grid cell). It carries a coverage ledger: a term with no OpenSCAD equivalent (a blend, a stroke, noise, an expr, a warp) arrives as a frozen \`polyhedron()\` and is named. Say "mojulo speaks OpenSCAD", never "mojulo replaces it" — and note the round trip ends there, since OpenSCAD writes no format \`bind_mesh_render\` accepts. The connective tissue is scale-honesty plus \`verify_machina\`, which checks work conservation, force capacity, rate and storage margins over a mechanism chain — statics, not collisions or joint limits.
...
- \`update_sketch\` — revise an existing sketch in place (rename, replace manifest, move folder) so the index doesn't accumulate near-duplicate refs. The ITERATE surface for every recipe stored as a sketch: diagrams (same validation as \`create_sketch\`), worlds / solids / figures / edifices / views (validated by resolving through the world registry — the render contract), and \`kind:'game'\` manifests (create_game's structural gate; newly added levels are noted as unaudited). Beats/voice refuse and point at their domain tools. Full-manifest replace: read, edit, write back.
