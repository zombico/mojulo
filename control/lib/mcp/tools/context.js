 ...
 *   operations-mode.js, worked-examples.js, and this file. If you add or remove a tool, update the relevant rows here
...
**Create things**
- First flight of a paradigm ("how does a bot/app/game actually get built end to end?", or you're about to run a flow you've never run) → \`get_worked_example({ paradigm })\` — the annotated trace with the gate moments marked; cheaper than trial-and-error.
...
- Automate over installed MCPs with **no chatbot** ("every Monday summarize X into Y", "when X in MCP-A do Y in MCP-B", "use mojulo without a bot") → \`meta_context_declare_inventory\` first (richer-snapshot mode — include \`inputSchema\`s — whenever you can read them). Then one rule decides the composer: **schemas declared → \`bind_primitives\`** (fills a role template with the operator's actual installed tool schemas); **first encounter with a provider you lack schema knowledge for → \`recommend_mcp_orbit_compositions\`** (the seed-reasoning scaffold). A host-adapter Skill via \`get_catalyst\` is the other Connected-Service form. Mojulo is the deliberation anchor + audit trail here, not the runtime.
...
- **Mint a DIAGRAM or data chart** ("draw me X", "chart these numbers", "a flow of Y") → \`create_sketch\` (chart vocab via \`semantic_search({kinds:['sketch_vocab']})\` + \`get_sketch_vocab\`); iterate in place with \`update_sketch\`; compare two with \`diff_sketches\`. Sketches persist at \`/sketches/<ref>\` and embed into picture-book / comic / field-guide / visual-guide cooks via \`sketch\`-typed stash items.
- **Mint a scene / figure ILLUSTRATION** ("illustrate X", a portrait, a painterly landscape) → \`sketch_what_possible\` knob-resolution loop → \`create_sketch({recipe:{kind,...}})\`; single-shot natural language → \`create_polygonized_sketch\`; painted landscape → \`compose_world\` (base \`painted-landscape\`; glyphs via \`semantic_search({kinds:['painted_landscape']})\`, glyph ids in \`overrides\`); cardinal-grammar structural illustration → \`create_manji_tree\` (2D or 3D).
- **Pose a HUMAN FIGURE** ("pose a figure / a person reaching / a walking figure / a male|female body in a stance / a figure wearing X") → \`create_figure\` (pose / build / garment / view dials; \`motion:'walk'\` adds a gait GIF).
- **A carved 3D WORDMARK / logo / badge** ("chrome text / extruded logo / beveled icon / metalify this shape") → \`create_carved_solid\`.
- **A single everyday OBJECT / mechanical part at literal scale** ("render/turntable a candlestick / bottle / dumbbell / vase / lamp; block out an object in solids") → \`create_workbench\` (a polygomer of \`lathes\` → a measured object at \`/world\` + preset shots at \`/scene\`); assemble finished parts → \`create_assembler\`; a single convex solid spinning live → \`create_solid_turntable\`.
- **A generated or LIVE WORLD** ("make a city / an airport / a walkable world / fly a drone / a platformer / a game where… / show me my services as a world / walk Königsberg") → \`compose_world\`: a BASE × a THEME. Bases: \`city\`, \`transport-hub\`, \`controllable\` (drive/fly/platform), \`action\` (games with rules via an \`idioms\` recipe), \`operator\` (mojulo's own state), \`planetary\`, \`painted-landscape\`, \`math\` (a group as a walkable Cayley city). Read the base's manual via \`get_view_vocab({id:'<base>'})\`; find one by intent via \`semantic_search({kinds:['view_vocab']})\`; themes via \`list_world_themes\`. Served traversable at \`/world\`.
- **An animated science / math / bio STUDY OBJECT** ("show/teach me nuclear fission / the double-slit experiment / what a derivative is / DNA / a black hole") → \`create_view\` (45 kinds; find one by intent via \`semantic_search({kinds:['view_vocab']})\`, read its parameter manual via \`get_view_vocab\`, then pass \`kind\` + \`params\`; served live at \`/world\`).
- **Mint AUDIO / make music or sound** ("give this world music / a soundtrack", "compose a tune / a jingle / a fanfare", "make a beat / a drum pattern / a house groove", "a pickup / laser / impact sound") → \`create_beats\` — synthesized WebAudio from a tiny deterministic recipe (no media bytes; plays at \`/sketches/<ref>\`). Four kinds: \`beats-ambient\` (seeded generative loop — the world-soundtrack primitive), \`beats-composition\` (explicit score), \`beats-pattern\` (step-sequencer groove), \`beats-sfx\` (chiptune foley gestures). Find a kind by intent via \`semantic_search({kinds:['beats_vocab']})\`, read its manual via \`get_beats_vocab\`, then pass \`params\`. Wire into a world via the world manifest's \`audio\` channel (soundtrack / wind / gait footsteps / event-bus \`sound:\` cues).
...
- **Put info in motion / EXPLAIN A CONCEPT visually** ("play these charts / a slideshow / a deck / an explainer / a report in motion") → \`forge_motion\` DECK family: \`subject.deck\` (ordered \`sk_…\` sketch refs and/or inline manifests, ≥2) or \`subject.stash_ref\` (a stash's \`sketch\`-typed items, in gather order) with \`shot.motion:'deck'\`; a slide's marks carry \`reveal:{step,…}\` to BUILD in sequence — graduated, paced disclosure for charts / diagrams / text. FORKS: a concept that IS a 3D object needing rotation (molecule / lattice / mechanism) → the CAMERA family (manji-tree turntable); a SINGLE convex solid → \`create_solid_turntable\` (live CSS-3D, no bake); one animate-annotated slide (\`animate:{channel:…}\`) renders smooth 2D motion live. Limits: no sound; smooth motion across a multi-slide deck stays staged. Chem / structure modeling guidance (ball-and-stick lathes, \`vajra\` bonds, \`taiji\` helices) lives in the \`forge_motion\` row of \`get_tool_index\`.
