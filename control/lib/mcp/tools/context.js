...
- \`status_app\` — read the runner's current in-memory state for one running_ref: \`{ status, url, mcp_url, artifact_ref, started_at }\`. \`status\` ∈ \`'running' | 'crashed' | 'stopped' | 'unknown'\`. Restart loses tracking — see notes on \`list_running\`.
- \`list_running\` — list all current running_refs known to this runner process: \`[{ running_ref, artifact_ref, status, url, mcp_url, started_at }]\`. **Control-plane restart drops in-memory state** — apps that survived the restart show as \`unknown\` until next \`stop_app\` and re-\`start_app\`. v0 scope; restart-survivable runner tracking is a follow-up.
