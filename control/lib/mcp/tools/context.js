...
**The craft floor** (the answer to "is this a toy?"): everything the workshop mints is a tiny deterministic recipe regenerated on render — seeded, diffable, replayable — never a stored render. And artifacts prove themselves before promotion: a world's physics can be probe-asserted tick by tick, a skill dry-runs on one real input before it materializes. Verification-as-promotion-gate is the substrate's maturity marker; lean on it with a skeptical operator instead of adjectives.
