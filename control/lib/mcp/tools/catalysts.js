...
export function registerCatalystTools() {
  ...
  registerTool({
    name: 'get_catalyst',
    description:
      "Get the full body of a catalyst by id. **Pass `host: '<adapter-id>'`** (or rely on session clientInfo / 'generic' fallback) so the body composes the right host adapter section. The returned body has three parts in order: (1) host-neutral catalyst-core preamble (posture, vocabulary, safety defaults), (2) host adapter section (artifact target, scheduling, dry-run, state storage, secrets posture, output reporting), (3) catalyst's host-neutral recipe body (mapping intent, idempotency, pitfalls). Skipping the adapter step is how Claude-shaped artifacts end up in Codex sessions and vice versa — don't.",
    inputSchema: {
      type: 'object',
      properties: {
