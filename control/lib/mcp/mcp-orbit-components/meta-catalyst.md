...
You are about to assemble an **mcp-orbit composition**: a workflow that reads from the operator's installed MCP environment (a source like Linear/Gmail/etc.), does something cognitive in the middle (aggregate, route, enrich), and writes to another MCP (a destination like Drive/Notion/Slack). Mojulo provides the components and constraint validation; you provide the judgment.
...
Every composition picks one component from at least the first five categories. The sixth (`render`) is optional in v0 (markdown is the default).

| Category | What it captures | Required? |
|---|---|---|
| `source` | Per-MCP read affordance: discovery, iteration, ID space, cursor mechanism. | yes |
| `destination` | Per-MCP write affordance: create / draft / append / send, required fields, dedupe surface. | yes |
...
3. **`pattern: branching` requires ≥2 destination components of distinct categories.** Without two destinations, "branching" is just "writing" — pick `pattern: enrichment` or `pattern: routing` instead.
4. **`pattern: aggregation` requires the source to expose either a window query or a cursor field.** Otherwise "aggregate the past week" has no semantics.
5. **An operator-KYC constraint forbidding `PII to LLM` forbids any `render` component that summarizes raw bodies.** Drop to `title+url+one-line` depth or use a structured render that doesn't pass body text through.
6. **An operator-KYC constraint naming a preferred MCP (e.g. "all docs go to Notion") clamps the destination at composition time.** Operator KYC overrides component defaults — never the other way around.
7. **Inventory must include an MCP matching the source's `requires.inventoryServerHints` and the destination's `requires.inventoryServerHints`.** No matching MCP = no valid composition; recommend installing one or surface the gap to the operator as a soft suggestion (consultation posture, not blocking).
...
1. **Inventory fit (highest weight).** Compositions whose source AND destination map cleanly to installed MCPs rank first.
2. **Operator-KYC alignment.** Compositions whose components are explicitly named or implied by the operator's locked-in constraints (e.g. KYC says "weekly Linear digest" → `source/linear + pattern/aggregation + trigger/scheduled` is a near-exact match) rank above generic fits.
...
4. **Pull each component via `get_mcp_orbit_component({ ref })`.** Read the body in full. Don't skim the pitfalls — they're load-bearing.
...
7. **Promote → host-adapter materialization → `meta_context_commit({type:'artifact_materialization', ...})`.** The commit's `bindings` include every component_ref the composition used (`{ kind, ref, version }` triples in the bindings payload). The commit's principles capture the negotiated knob values. Update the composition row to `status: materialized` and set `artifact_ref` to the artifact node's composite ref.
...
3. **Write one real (reversible) destination artifact.** Permissions, quotas, field validation fail at the destination, NOT in your render code. Use the destination's draft affordance (Docs: leave unpublished; Notion: don't share; Slack: send to a test channel). Surface the URL and ask for promotion confirmation.
...
- `bindings[]` — one entry per source/destination MCP tool the artifact calls (e.g. `linear.list_issues`, `gdrive.create_file`). The `fields_bound` array names the actual fields the composition reads / writes.
- Record the **composition ref** in an artifact-scope principle: `"Composed from components: source/linear@0.1.0, destination/gdrive@0.1.0, trigger/scheduled@0.1.0, idempotency/window-key@0.1.0, pattern/aggregation@0.1.0. Composition ref: comp_<id>. Knobs: cadence=weekly, day=Mon, depth=title+url+one-line, ..."` — this is the durable link from the materialized artifact back to its composition row.
