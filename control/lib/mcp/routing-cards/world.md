...
---
→ `compose_world`: a BASE × a THEME. Bases: `city`, `transport-hub`, `controllable` (drive/fly/platform), `action` (games with rules via an `idioms` recipe), `operator` (mojulo's own state), `planetary`, `painted-landscape`, `math` (a group as a walkable Cayley city). Read a base's manual via `get_view_vocab({id:'<base>'})`; find one by intent via `semantic_search({kinds:['view_vocab']})`; themes via `list_world_themes`. Served traversable at `/world`. (Contrast: a playable game with a persistent store → `create_game`, which promotes worlds into levels.)
