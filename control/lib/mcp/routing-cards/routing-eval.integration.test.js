...
// forward-context-grammar.plan.md) so both gates ride the same phrasings.
// When adding a creative capability, add 1–2 rows THERE.
import { FIXTURE, COLLISION_FIXTURE } from './routing-eval.fixture.js';
...
// Adjacency collisions (COLLISION_FIXTURE, imported above): pairs of cards
// that share heavy surface vocabulary where the top-3 membership gate above is
// too loose — the WRONG neighbour can sit at RANK 0 while the right card rides
// along at rank 1 and the top-3 gate stays green. The collision test pins
// RANK-0 (true top-1) for the intent: the gate that actually catches a
// mis-route. Seeded from the persona-sweep loop that found "playable dungeon
