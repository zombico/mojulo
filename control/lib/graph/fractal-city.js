...
import { pedestrianFaces, IDLE_POSES, STROLL_POSES, PALETTES } from './pedestrian-asset.js';
...
function parkBench(boxes, x, y, axis, rng) {
  const wood = rng() < 0.5 ? '#8a6c43' : '#9a7b4e', frame = '#3d444b';
  const L = 1.42, B = 0.46;
  paBox(boxes, 'park-bench', axis, x, y, -L / 2, L / 2, -0.18, -0.04, 0.32, 0.38, wood);           // seat plank (front)
  paBox(boxes, 'park-bench', axis, x, y, -L / 2, L / 2, 0.02, 0.16, 0.32, 0.38, wood);             // seat plank (rear)
  paBox(boxes, 'park-bench', axis, x, y, -L / 2, L / 2, B / 2 - 0.06, B / 2, 0.42, 0.52, wood);    // backrest plank (lower)
  paBox(boxes, 'park-bench', axis, x, y, -L / 2, L / 2, B / 2 - 0.06, B / 2, 0.58, 0.70, wood);    // backrest plank (upper)
...
function scatterParkDoodads(leftover, grid, boxes, grounds, faces, rng) {
  for (const comp of leftover) {
    ...
    for (let i = 0; i < benches; i++) { const s = placeOnGrass(grid, comp, benchFW, benchFD, rng); if (s) parkBench(boxes, s.x, s.y, benchAxis, rng); }
