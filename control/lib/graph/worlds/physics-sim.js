...
export function buildSim() {
  ...
  function normalizeBody(raw, index) {
    ...
    // Optional TETHER (a rigid pendulum rod): a dynamic body pinned to `anchor` at a fixed distance.
    // Enables interactive pendulums / Newton's cradles without a general constraint solver — it is a
    // single distance constraint, projected each step (see step()). Length defaults to the rest gap.
    if (Array.isArray(raw.anchor)) {
      body.anchor = vcopy(raw.anchor);
      body.tether = Number.isFinite(raw.tether) && raw.tether > 0 ? raw.tether : len(sub(body.position, body.anchor));
    } else {
      body.anchor = null;
    }
    return body;
  ...
  function step(state, dt) {
    ...
    // TETHER constraint pass (after collisions): project each tethered body back onto its rod sphere
    // and remove the radial velocity component — a rigid pendulum. At a cradle's rest the rod is
    // vertical and collisions are horizontal (orthogonal), so momentum transfer is preserved.
    for (let k = 0; k < bodies.length; k++) {
      const body = bodies[k];
      if (body.invMass === 0 || !body.anchor) continue;
      const d = sub(body.position, body.anchor);
      const dist = len(d);
      if (dist < 1e-9) continue;
      const n = scl(d, 1 / dist);
      body.position = add(body.anchor, scl(n, body.tether));
      const vr = dot(body.velocity, n);
      if (vr !== 0) body.velocity = sub(body.velocity, scl(n, vr));
    }
    const touched = {};
