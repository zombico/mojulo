...
def unlit_instance(master, tex, cache):
    ...
    if key in cache:
        return cache[key]
    name = 'MI_Mojulo_' + (tex.get_name() if tex is not None else 'White')
    path = MAT_ROOT + '/' + name
    if unreal.EditorAssetLibrary.does_asset_exist(path):
...
def apply_unlit_materials():
    """Swap every imported mesh slot onto the unlit vertex-colour look.
    Idempotent: slots already on a Mojulo material are left alone."""
    master = ensure_unlit_master()
    cache = {}
    swapped = 0
    ...
    unreal.log('[mojulo] unlit vertex-colour materials on ' + str(swapped) + ' slots')
 ...
 */
export function emitUnrealProject({ ref, score, manifestHash, audioFile = null, remint = null }) {
 ...
 */
export function emitUnrealGame({ ref, manifest, levels, manifestHash, remint = null }) {
