...
function __makeBody(e) {
  ...
  if (b.type === 'figure-frames' && __FIG[b.figure]) {
    ...
    const clips = {};   // clip name → array of per-frame geometries (one clip per locomotion mode)
    for (const cn in fig.clips) { const cd = fig.clips[cn]; clips[cn] = cd.pos.map((p, i) => __buildFigGeo(p, cd.col[i], cd.origin, cd.invScale, cd.foot)); }
    const first = clips.forward || clips[Object.keys(clips)[0]];
    const mesh = new THREE.Mesh(first[0], new THREE.MeshBasicMaterial({ vertexColors: true, side: THREE.DoubleSide }));
    const group = new THREE.Group(); group.add(mesh); scene.add(group);
    group.userData.fig = { clips, mesh, yaw: b.yawOffset != null ? b.yawOffset : -Math.PI / 2 };   // figure faces +y → forward
    return group;
  }
...
function __syncEntity(e) {
  ...
  if (fig) {
    ...
    const clip = (e.locomotion && fig.clips[e.locomotion]) || fig.clips.forward || fig.clips[Object.keys(fig.clips)[0]];   // clip by locomotion mode
    const N = clip.length, ph = ((e.gaitPhase % 1) + 1) % 1;    // frame chosen by stride phase
    const frame = e.moving ? (Math.floor(ph * N) % N) : 0;
    if (fig.mesh.geometry !== clip[frame]) fig.mesh.geometry = clip[frame];
  } else {
...
export function emitRaymarchWorld({ frag, customUniforms = {}, dataTextures = {}, cameraStart = [0, 3, 17], target = [0, 0, 0], fov = 46, readout = [], steps = [], viewBox = { width: 1120, height: 780 }, title = 'mojulo world', bg = '#01010a', inline = false, cdn = false, pixelRatioCap = 1.5 } = {}) {
  ...
  const td = Object.entries(dataTextures).map(([k, t]) =>
    `${k}:{value:(()=>{const tx=new THREE.DataTexture(new Float32Array(${JSON.stringify(t.data)}),${t.width},${t.height},THREE.RGBAFormat,THREE.FloatType);tx.minFilter=THREE.NearestFilter;tx.magFilter=THREE.NearestFilter;tx.needsUpdate=true;return tx;})()}`,
  ).join(',');
...
function eventsChannelScript(events) {
  return `
...
export function emitThreeWorld({ faces = [], cameras = [], viewBox = { width: 1120, height: 780 }, title = 'mojulo world', bg = '#0e1014', inline = false, cdn = false, glow = true, light = null, sky = null, textures = {}, wireframe = false, walk = false, picks = [], tracers = [], movers = [], comets = [], fields = [], surfaces = [], buildups = [], transports = [], deforms = [], raymarch = null, decollide = true, capture = false, signs = [], physics = null, actions = [], entities = [], camera = null, figures = {}, events = null, fog = null } = {}) {
  ...
  const __packClip = (frames) => {
    ...
    return { pos: pk.pos, col: pk.col, origin: pk.origin, invScale: pk.invScale, foot, figH };
  };
...
${actionsBlock}
// events channel (opt-in, LIVE): the in-world bus. stepEvents stays inert unless an eventsBlock is
...
function __mojStep(t) { stepTracers(t); stepMovers(t); stepComets(t); stepFields(t); stepSurfaces(t); stepBuildups(t); stepTransports(t); stepDeforms(t); stepSigns(t); stepPhysics(t); stepEvents(t); }
${fog ? `
...
const __fogMat = new THREE.ShaderMaterial({ uniforms: __fogU, vertexShader: 'void main(){ gl_Position = vec4(position.xy, 0.0, 1.0); }', fragmentShader: ${JSON.stringify(fog.frag)}, transparent: true, depthTest: false, depthWrite: false, blending: THREE.CustomBlending, blendSrc: THREE.OneFactor, blendDst: THREE.OneMinusSrcAlphaFactor });
...
__fogQuad.onBeforeRender = (rnd, scn, cam) => {
  ...
  __fogU.uTime.value = (typeof performance !== 'undefined' ? performance.now() : 0) / 1000;
};
scene.add(__fogQuad);
` : ''}
