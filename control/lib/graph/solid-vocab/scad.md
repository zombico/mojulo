...
- `color()` — becomes the face `tint`; the studio light shades it exactly as a workbench `tint`. Uncoloured geometry is a neutral grey. (OpenSCAD's own STL drops colour; mojulo's mesh keeps it, and the `.glb` carries it as vertex colour.)
...
- **A sculpted cut:** `difference() { body(); color("#8a9bb0") translate([0, 0, 14]) mojulo_field("dent"); }`. The faces a cutter leaves behind wear the CUTTER's colour, so colour the field call to match the body (an uncoloured cutter leaves OpenSCAD's default yellow). Shape the field with `stroke` and `blend`; a heavy `displace` folds the surface net and is refused at mint.
