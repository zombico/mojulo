...
- **fields** — named fields (`{ <id>: { kind, ... } }`) that position-dependent parameters (currently `waveManji[].bending`) reference via `{ field: '<id>' }`. The full closed kind set is `constant | radial | gradient | wave-surface | noise | sum | curve-projection (vector) | curve-distance | terrain-region` — see the fields section of docs/POLYGONIZER-SYNTHESIS.md for the per-kind knobs.
