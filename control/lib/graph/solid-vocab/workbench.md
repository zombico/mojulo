...
**The edge caveat.** The surface net is not a mesh CSG kernel: every edge and corner rounds to about ONE GRID CELL (the longest side ÷ `cells`), and the stamped face count grows with the square of `cells` while the cost grows with its cube. **`exact: true` on the entry lifts that inside the recipe**: the same term list is composed by Manifold (the boolean kernel under OpenSCAD 2025, an optional creative dependency), so a bore has a sharp lip in /world, in the `.glb` and on the printer, and a 12-unit disc measures 12, not 11.9. Its reach is the nine shapes, `add` / `subtract` / `intersect` without `blend`, `transform` and `repeat`; a `blend`, `stroke`, `displace`, `shell`, `round`, `expr`, harmonic lathe or warp is refused at mint by name (drop `exact`, or shape that detail as its own sampled entry). `segments` (8–256, default 48) facets the curved primitives; `cells` is ignored. Two more exits give a sharp edge on the way out: `export_model union:true` (Manifold) unions shells sharply at export, and `export_model format:'scad'` transpiles the recipe into an OpenSCAD program — the same solids and booleans as code, recomputed by an exact kernel, so a bore arrives with a sharp lip. A chamfer is still the DCC's. The `scad` result carries a coverage ledger: terms with no OpenSCAD equivalent (a `blend`, a `stroke`, `displace`, an `expr`, a twist / bend / taper) arrive as a frozen `polyhedron()` and are named.
...
- `exact` (bool) — compose this entry with Manifold instead of the grid (see the edge caveat above): sharp edges, exact size, one triangle per facet. `segments` (int 8–256, default 48) — facets around a curved primitive under `exact`.
...
- `movers` (top level, beside the monomers) — the World's mover channel on those groups. A hinge is a `turn` about `axis` through `center`, in radians; `absolute: true` keeps the group's authored world coordinates (the recipe, the still and every export show the authored pose; only the live World swings it). Two ways to drive it:
  - a **toggle** — `{ group: 'lid', label: 'lid', basePos: [0,0,0], turn: { center, axis, absolute: true }, states: [0, -1.83], key: 'o', transition: 0.7 }`. `states` lists the resting angles: two by default (closed, open), add a third for a half stop or as many as the part has detents. Clicking the part or pressing `key` steps to the next state (shift+key steps back; `window.__mojToggle(group, dir)` from script); `transition` eases the swing (smoothstep by default; `ease: 'snap'` is the magnetic latch — the part eases out to a hover just short of the detent, then the last stretch accelerates in and clicks; `ease: { snap: 0.2, gap: 0.1 }` dials the time share and arc share of that click; an array is read by the state arrived at — `ease: ['snap', null]` clicks into state 0 and swings plainly into state 1). State 0 holds until an input arrives, so captures and turntables stay deterministic. `click: false` leaves only the key.
