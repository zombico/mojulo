 ...
 * @param {object|null} [opts.occluder]    a solid the march stops at, shaded by `groundFn`
 *                                          (a `vec3 fn(vec3 p, vec3 rd)`). Two forms:
 *                                            • SPHERE — `{ radiusUniform, groundFn }`: an inner sphere
 *                                              (a planet, a horizon).
 *                                            • SCENE SDF — `{ sdfFn, groundFn, traceSteps?, surfEps?, minStep? }`:
 *                                              the ray sphere-traces `float sdfFn(vec3)` to its first surface.
 *                                              This is how the effects layer clips fog against an arbitrary
 *                                              solid world (a town's union-of-boxes, a cave). `traceSteps`
 *                                              (default 96), `surfEps` (default '0.004'), `minStep`
 *                                              (default '0.02') tune the trace cost vs surface accuracy.
...
export function buildVolumeFrag({
  ...
  // Optional occluder: stop the march at the first solid surface and shade the hit instead of the
  // background. Two forms — a single inner SPHERE (radiusUniform), or a SCENE SDF (sdfFn) the ray
  // sphere-traces, which is how the effects layer clips fog against a town's union-of-boxes / a cave.
  const sdfOcc = occluder && occluder.sdfFn;
  let occSetup = '';
  let occClip = '';
  let bgComposite = `  col += trans * ${bg};                                     // background, occluded by the volume`;
  if (sdfOcc) {
    const traceSteps = Math.max(1, Math.floor(occluder.traceSteps ?? 96));
    const surfEps = occluder.surfEps ?? '0.004';
    const minStep = occluder.minStep ?? '0.02';
    // sphere-trace the scene SDF within the bounds interval [t0, t1]; clip the volume march there.
    occSetup = `  bool hitGround = false; float tHit = 0.0;\n`;
    occClip =
      `      { float ts = t0;\n` +
      `        for (int gi = 0; gi < ${traceSteps}; gi++) {\n` +
      `          float d = ${occluder.sdfFn}(ro + rd * ts);\n` +
      `          if (d < ${surfEps}) { hitGround = true; tHit = ts; break; }\n` +
      `          ts += max(d, ${minStep});\n` +
      `          if (ts > t1) break;\n` +
      `        }\n` +
      `        if (hitGround) t1 = min(t1, tHit); }\n`;
    bgComposite = `  col += trans * (hitGround ? ${occluder.groundFn}(ro + rd * tHit, rd) : ${bg});`;
  } else if (occluder) {
    occSetup = `  vec2 tg = vrSphereT(ro, rd, ${occluder.radiusUniform});\n  bool hitGround = (tg.y > tg.x) && tg.x > 0.0;\n`;
    occClip = `      if (hitGround) t1 = min(t1, tg.x);\n`;
    bgComposite = `  col += trans * (hitGround ? ${occluder.groundFn}(ro + rd * tg.x, rd) : ${bg});`;
  }
  ...
  const marchLoop =
    ...
    `      trans *= exp(-ext * dt);                              // front-to-back absorption\n` +
    `      t += dt;\n` +
    `      if (max(trans.r, max(trans.g, trans.b)) < 0.01) break;\n` +
    `    }\n`;

  // OVERLAY: a transparent fog pass composited OVER an already-rendered mesh world (the effects
  // channel). No bounding sphere — march [0, uMaxDist] clipped by the scene-SDF occluder; output
