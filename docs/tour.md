# Mojulo, the long tour

> The long-form README as it stood at v2.0.4. The repo README is now a short set of worked examples; this page keeps the full catalog of what mojulo makes, who it is for, and the reasoning behind the posture.
...
Mojulo is a 3D compiler for agents: a local MCP server where everything your coding agent makes is a small recipe it can re-run identically.
...
![A described mug — two lathes and a swept handle — gravity-seated on a table, shown as flat albedo on mojulo's measured studio grid under the default house light](images/object-bake-before.jpg) ![The same object recipe after one optional trip through a local Blender Cycles bake — traced global illumination baked into the geometry's own vertex colours, grounded contact shadows, studio backdrop](images/object-bake-after.jpg)
...
- **Blender** — the GI bake into vertex colours, premium stills, and the USD/GLB verify gate. [docs/local-blender-worker.md](local-blender-worker.md)
- **A slicer** — PrusaSlicer, Bambu Studio (OrcaSlicer is detected but not yet run) for the print gate. [docs/local-slicer-worker.md](local-slicer-worker.md)
- **A mesh sculptor** — TripoSR, Hunyuan3D or a cloud API for high-frequency surface detail over mojulo's greybox; the result binds back with provenance. [docs/local-mesh-worker.md](local-mesh-worker.md)
- **An image painter** — for directed images only: an image-capable agent, or a self-hosted ComfyUI + Qwen worker (large models, loopback only). [docs/local-image-worker.md](local-image-worker.md)
...
- [docs/POLYGONIZER-SYNTHESIS.md](POLYGONIZER-SYNTHESIS.md) — the geometry substrate: wave primitives, structure-manji, field kinds, shelf cards
- [docs/HTML-CSS-NATIVE-RENDERING.md](HTML-CSS-NATIVE-RENDERING.md), [docs/scene-css3d-lighting.md](scene-css3d-lighting.md) — the dependency-free scene backend and its baked lighting model
- [docs/raymarch-effects-layer.md](raymarch-effects-layer.md) — volumetric effects as an overlay over the mesh worlds
- [docs/bicycles.md](bicycles.md) — the two-gate doctrine (machine gate, eyes gate) every handoff runs
- [docs/tech-requirements.md](tech-requirements.md) — measured footprint, engine legs, slicers, workers, platform notes
- [docs/examples/unreal-night-run/](examples/unreal-night-run/) — the Unreal worked example: recipes, mint script, project-side wiring
- [docs/local-blender-worker.md](local-blender-worker.md), [docs/local-slicer-worker.md](local-slicer-worker.md), [docs/local-mesh-worker.md](local-mesh-worker.md), [docs/local-image-worker.md](local-image-worker.md), [docs/local-voice-worker.md](local-voice-worker.md) — the optional operator-hosted workers
