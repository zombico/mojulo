...
**Federated routing.** A triage handoff also carries the sender's tip-of-chain through the URL so the receiver's first turn descends from it, and records a chained `handoff` event row on the sender via `navigator.sendBeacon`. The result is end-to-end tamper-evidence across bot boundaries — each bot keeps its own SQLite, neither reads from the other, but the chain math is continuous. See [docs/chatbot/federated-routing.md](federated-routing.md).
