...
2. **The right-hand panel runs the bot's actual client, not a mockup.** The Theatre iframe is fed wizard state through the preview-shim ([docs/chatbot/bot-frontend.md](bot-frontend.md)) — the same `index.html`, `marked` rendering, ghost-form pipeline, triage cards. Tweaks reflect immediately, no rebuild between iterations. What you preview is structurally what you ship.
...
- [docs/chatbot/bot-frontend.md](bot-frontend.md) — the bot client the Theatre's preview iframe runs
- [docs/chatbot/vector-rag.md](vector-rag.md) — what the Knowledge step's embedding run actually produces
- [docs/chatbot/form-collection.md](form-collection.md) — what the Form Gathering step's locale-aware schema generation feeds into
- [docs/chatbot/federated-routing.md](federated-routing.md) — what the Triage step's routes become at runtime
