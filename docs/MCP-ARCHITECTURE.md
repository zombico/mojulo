...
| `meta_embeddings` | semantic recall | One row per `(source_kind, source_ref)` across the indexed kinds declared in the table's CHECK constraint. Raw `float32[384]` BLOB embedding, `content_hash` skip-on-unchanged, `model` column reserved for future model swaps. |
