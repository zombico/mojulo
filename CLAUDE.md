...
Tool registration is lazy ([`ensureToolsRegistered`](control/lib/mcp/server.js)) and ordered — `forward_context` first, fleet between per-bot operate and catalysts, then Ring 6 in the order contextmap → inventory → capabilities → composer → primitive-binding → trigger-binding → semantic-search, then Ring 7 in the order runner → agent-tasks. Natural reading order: orientation → per-bot → fleet → outcome → deliberation (structured walks) → deliberation (fuzzy recall) → runtime. When you add a tool, slot it into the right ring and update the tool index in [context.js](control/lib/mcp/tools/context.js); the agent reads that index to disambiguate, so a missing entry leaves it flying blind.
